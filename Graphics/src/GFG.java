...
class GFG{
    ...
    // Function checks if a cell is valid i.e it is inside the grid and equal to the key (which is numbers in this question)
    static boolean is_valid(int x, int y, int key, int input[][]){
        //ensure x,y is in range, unvisited, and with the same color
        if (x < n && y < m && x >= 0 && y >= 0){
    ...
    static void BFS(int x, int y, int i, int j, int input[][]){
        ...
        // x_move and y_move array are the possible movements in x or y direction
        int x_move[] = { 0, 0, 1, -1 };
        int y_move[] = { 1, -1, 0, 0 };
